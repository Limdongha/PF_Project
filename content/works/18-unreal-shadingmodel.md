...
머티리얼 에디터에서 넣은 정보가 HLSL(GPU)까지 전달되는 흐름 전체를 **택배 배송에 비유**해 설명하겠습니다. 정보를 포장해서 주소지로 보내고 도착지에서 열어 처리하는 구조가 실제 파이프라인과 잘 맞아떨어집니다.
...
다음은 이 택배가 잘 가고 있는지 추적할 수 있도록 표시를 다는 작업입니다. 없어도 동작은 하지만 디버깅을 하려면 사실상 필수입니다.

...
에너지 보존은 나가는 빛이 들어온 빛보다 많을 수 없다는 원칙입니다. 그래서 표면에 닿은 빛을 반사(kS)와 확산(kD)으로 나눠 씁니다.

- **kS** — 프레넬 결과. 스펙큘러 BRDF 안에 이미 포함되므로 따로 곱하지 않습니다.
- **kD** — `(1 - kS) × (1 - metallic)`. 금속은 디퓨즈가 없습니다.

이 분배는 두 항이 에너지를 중복으로 쓰지 않게 하는 장치입니다.

## 4. 메탈릭 워크플로 — F0 한 줄로 갈라지는 것

파이프라인은 인풋을 스펙큘러로 받느냐 메탈릭으로 받느냐로 나뉩니다. 스펙큘러를 받으면 F0에 그대로 대입하면 되고 메탈릭을 쓰면 알베도와 메탈릭으로 F0를 만들어 냅니다. 저는 메탈릭 워크플로를 택했습니다. 메탈릭은 1채널이라 러프니스·AO와 한 텍스처에 패킹할 수 있는데 스펙큘러는 RGB가 통째로 필요합니다. 메모리와 대역폭에서 차이가 커서 이쪽을 골랐습니다.
...
F0는 표면을 정면에서 봤을 때 반사되는 빛의 비율입니다. 비금속은 약 4%의 무채색 반사를 하고, 색은 빛이 내부에서 산란하다 나오는 디퓨즈에서 생깁니다. 금속은 내부로 들어간 빛을 모두 흡수해 디퓨즈가 없고, 대신 표면 반사 자체에 색이 있습니다. 그래서 금속일수록 알베도가 F0로 옮겨가고, 디퓨즈 색은 `albedo × (1 - metallic)`로 줄어듭니다.
...
셰이더 코드는 직접 작성했습니다. 개념을 정리하면서 참고한 글들입니다.

- 미세면 이론과 에너지 보존 그리고 Cook-Torrance BRDF 전반 — [PBR(물리 기반의 렌더링) 기본 정리](https://bbtarzan12.github.io/PBR/)
- D·G·F 세 항을 나눠 본 설명 (GGX·Smith·Schlick) — [[그래픽스][PBR] BRDF](https://velog.io/@15ywt/%EA%B7%B8%EB%9E%98%ED%94%BD%EC%8A%A4PBR-BRDF)
- 메탈릭 워크플로와 스펙큘러 워크플로의 차이 — [[컴퓨터 그래픽스] PBR과 주요 워크플로우](https://velog.io/@sumc00/%EC%BB%B4%ED%93%A8%ED%84%B0-%EA%B7%B8%EB%9E%98%ED%94%BD%EC%8A%A4-PBR%EA%B3%BC-%EC%A3%BC%EC%9A%94-%EC%9B%8C%ED%81%AC%ED%94%8C%EB%A1%9C%EC%9A%B0)

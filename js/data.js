...
const WORKS = [
...

  {
    ...
    study: [
      { p: "DirectX 11로 직접 구현한 렌더러 위에, 1인 개발로 모작한 게임에 넣기 위해 만든 파티클 시스템입니다." },
      { p: "먼저 그리는 뼈대를 세우고 그 위에 파티클의 움직임 → 겉모습 → 수명 → 태어나는 위치를 하나씩 얹어 갔습니다. 아래는 그 순서대로 정리한 과정입니다." },

      { h: "1. 드로우콜을 줄이는 인스턴싱 구조" },
      { p: "물체 하나를 그릴 때마다 CPU가 GPU에게 '이걸 그려라'라는 명령을 보내는데 파티클을 하나씩 그리면 1,000개를 그리는 데 명령이 1,000번 나갑니다. 이 호출 비용이 쌓이면 GPU가 놀고 있어도 CPU 쪽에서 병목이 생깁니다." },
      ...
      { img: "content/works/img/04-gpu-particle/12.png", cap: "무기 메쉬 표면에서 스폰된 빛 입자. 면적 가중치와 바리센트릭 좌표를 적용해 날 전체에 고르게 피어오릅니다." },
    ],
    links: [
